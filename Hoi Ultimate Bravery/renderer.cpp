...
void Renderer::renderStats(bool windowOpen, Ship ship, std::map<Hull::Type, std::string> shipIconNames, std::unordered_map<Hull::Type, Stats> newShipStats)
{
    ...
    for (auto& module : ship.fixedModule) {
        ...
        setImage(Constant::TextPos::TANK_MODULE_HEIGHT + 2.0f, getModulePos(index), "modules", fileName, UnitType::Type::Ship);
        index++;
    }
    index = 0;
    for (auto& module : ship.fixedModule) {
    ...
    /*std::string fileName = std::format("{0}_{1}", Module::typeToString(ship.customModule[0]), Gun::typeToString(tank.gun.type));
